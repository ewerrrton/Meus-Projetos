...
public class ArvoreRB {
    ...
    public void intersecaoRestrita(ArvoreRB a1, ArvoreRB a2) {
        intersecaoRestrita(a1.root, a2.root, 0);
    }

    public void intersecaoRestrita(Node n1, Node n2, int nivel) {
        if(n1 == null || n2 == null) {
            return;
        }

        if(n1.value == n2.value) {
            System.out.println("Nivel" + nivel + ": " + n1.value);
        }

        intersecaoRestrita(n1.left, n2.left, nivel+1);
        intersecaoRestrita(n1.right, n2.right, nivel+1);
    }
    ...
    public void printaNivel(Node node, int nivelAtual, int nivelDes) {
        if(node == null) {
            return;
        }

        if(nivelAtual == nivelDes) {
            if(node.cor == Node.Color.PRETO) {
                System.out.print(node.value + " ");
            }
        }

        printaNivel(node.left, nivelAtual +1, nivelDes);
        printaNivel(node.right, nivelAtual+1, nivelDes);
    }
